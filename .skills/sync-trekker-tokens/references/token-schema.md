...
Every collection (except Styles, which comes from text styles not variables) includes a `_meta` block for restore:

```json
{
  "_meta": {
    "collectionId": "VariableCollectionId:25:904",
    "modes": { "Light": "25:9", "Dark": "25:10" }
  }
}
```

Every token includes a `_figmaId` field and optionally `_aliasIds`:

```json
{
  "name": "background/base/primary",
  "_figmaId": "VariableID:25:905",
  "_aliasIds": { "Light": "VariableID:13:100", "Dark": "VariableID:13:200" },
  "light": { "hex": "FFFDF6", "alias": "base-00" },
  "dark": { "hex": "100F0F", "alias": "base-1000" }
}
```

Rules:
- `_figmaId`: The Figma variable ID string (e.g. `VariableID:25:905`). Used to look up and write back values during restore.
- `_aliasIds`: Only present when the token is an alias. Maps mode name → the aliased variable's ID. Used to restore `VARIABLE_ALIAS` references rather than setting resolved values directly.
- These fields are prefixed with `_` to signal internal metadata. The site UI ignores them.
  ...
  "changelog": [
    {
      ...
      "changes": [
        { "type": "added|changed|removed", "collection": "color", "group": "background", "name": "background/neutral/subtlest", "value": { "light": "F7F6ED", "dark": "1C1B1A" } },
        { "type": "changed", "collection": "color", "group": "icon", "name": "icon/accent/blue", "before": { "light": "AAA" }, "after": { "light": "BBB" } }
      ],
      "summary": { "added": 1, "changed": 14, "removed": 0 },
      "notes": "Optional freeform description of the sync."
    }
  ]
}
```
...
Rules:
- Entries are appended on each sync. The build script preserves existing entries.
- `changes` array items include `type`, `collection`, `group`, `name`, and optionally `before`/`after` (for changed) or `value` (for added).
- `summary` is a convenience rollup of counts.
- `notes` is optional freeform text generated by the sync skill or provided by the user.
